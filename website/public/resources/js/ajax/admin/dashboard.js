    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            jsonResponse.forEach((producto, index) => {
                let style =
                    producto["cantidad"] <= 500 && producto["cantidad"] >= 60
                        ? "bg-success"
                ...
                rows += `
                    <div class="row dash__side_card1_container  ${
                        index == jsonResponse.length - 1 ? "pb-4" : "pb-0"
                    } ${index == jsonResponse.length - 1 ? "pb-4" : "pb-0"}">
                        <div class="col-lg-3 col-md-3 col-sm-12 col-12">
                            <p class="dash__sc1_text text-truncate" title="${
                                producto.producto
                            }">${producto.producto}</p>
                        </div>
                        <div class="col-lg-9 col-md-9 col-sm-12 col-12 dash__bar">
                            <div class="progress">
                                <div class="progress-bar ${style}" role="progressbar" style="width: ${
                    producto.cantidad
                }%" aria-valuenow="${
                    producto.cantidad
                }" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                            <p class="dash__sc1c_text">${
                                producto.cantidad
                            } restantes</p>
                        </div>
                    </div>`;
        ...
        success: function (response) {
            if (response.status) {
                ...
                barGraph(
                    ...
                    cantidad,
                    "Cantidad",
                    "Cantidad de clientes registrados por día."
                );
            } else {
        ...
        success: function (response) {
            if (response.status) {
                ...
                doughnutGraph(
                    ...
                    cliente,
                    cantidad,
                    "Clientes con la mayor cantidad de ordenes"
                );
            } else {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == 1) {
                fetchResource(
                    "http://localhost/poseidon/website/public/reports/clientes.pdf"
                );
            } else {
...
function productosMasVendidos() {
    $.ajax({
        ...
        success: function (response) {
            if (response.status == 1) {
                fetchResource(
                    "http://localhost/poseidon/website/public/reports/productosmasvendidos.pdf"
                );
            } else {
                swal(2, response.exception);
            }
