...
function fillTable(dataset) {
    ...
    if ($.fn.dataTable.isDataTable(table)) {
        ...
        table.DataTable({
            ...
            columnDefs: [{ targets: -1, className: "td-actions text-center" }],
            columns: [
                { data: "idcliente" },
                { data: "nombre" },
                { data: "apellido" },
                ...
                { data: "estado" },
                {
                    ...
                    render: function (data) {
                        return data["enlinea"] == true
                            ? `<i class="fas fa-link" alt="Conectado"></i>`
                            : `<i class="fas fa-unlink" alt="Desconectado"></i>`;
                    },
                    targets: -1,
                },
                {
                    ...
                    render: function (data) {
                        ...
                        <div class="dropdown">
                            ...
                            <button class="dropdown-item" onclick="changeState(${data["idcliente"]}, ${data["estado"] != "Activo" ? 1 : 3}, this)" type="button">
                                <span>
                                    <i class="fas ${data["estado"] != "Activo"
                                            ? "fa-eye"
                                            : "fa-eye-slash"
                                    }">
                                    </i>
                                    <p>${data["estado"] != "Activo"
                                            ? "Hacer activo"
                                            : "Suspender"
                                    }</p>
                                </span>
                            ...
                            ${ data["enlinea"] == true ? `
                            <div class="dropdown-divider"></div>
                            <button class="dropdown-item" onclick="cerrarSesion('${data["email"]}', this)" type="button">
                                <span>
...
function cerrarSesion(email, el = false) {
    function before() {}
    function after() {}
    if (el) {
        ...
        function before() {
            if (buttons)
                buttons.innerHTML =
                    '<div class="spinner-grow" role="status"><span class="sr-only">Cargando...</span></div>';
        }
        function after() {
            if (buttons) {
    ...
    let text = "¿Desea cerrarle la sesión al usuario con correo " + email + "?";
    swal(
        4,
        text,
        false,
...
$("#clients-form").submit(function (event) {
    ...
    if ($("#inputEmail").is("[data-id]")) {
        saveRow(
            ...
            ["Nombre", "Apellido", "Email", "Dirección", "Teléfono", "Estado"],
            $("#inputEmail").attr("data-id"),
            false,
            cancelUpdate
        );
    }
...
function reporteordenes(idcliente) {
    $.ajax({
        type: "post",
        url: API_CLIENTES + "reporteOrdenes",
        ...
        success: function (response) {
            ...
            if (response.status == 1) {
                fetchResource(
                    "http://localhost/poseidon/website/public/reports/ordenescliente.pdf"
                );
            } else {
                swal(2, response.exception);
            }
